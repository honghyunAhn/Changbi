-- DATETIME에 DEFAULT CURRENT_TIMESTAMP 는 mysql 5.6버전 이상부터 가능
-- AUTO_INCREMENT 와 DEFAULT 는 5버전 이상부터는 같이 사용 불가
CREATE TABLE CB_SURVEY_CARDINAL (
	SEQ					INT(12)			AUTO_INCREMENT	UNIQUE		COMMENT '설문 기수 매핑 SEQ',
	SURVEY_ID			VARCHAR(14)									COMMENT '설문 ID',
	CARDINAL_ID			VARCHAR(12)									COMMENT '기수 관리 ID',
	SURVEY_CODE			VARCHAR(12)									COMMENT '설문분류코드(기초관리>설문분류)',
	PRIMARY KEY(SURVEY_ID, CARDINAL_ID)
) COMMENT='설문 기수 매핑 TABLE';