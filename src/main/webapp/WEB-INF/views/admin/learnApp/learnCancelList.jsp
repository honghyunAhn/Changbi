...
$(document).ready(function () {
	/** 변수 영역 **/
	// 여기에서 URL 변경 시켜서 사용
	var listUrl	= "<c:url value='/data/learnApp/learnCancelList' />";
	var cancelUrl = "<c:url value='/data/learnApp/learnCancel' />";
	var excelUrl = "";
			...
			success	: function(result) {
				...
				if(result.list && result.list.length > 0) {
					...
					for(var i=0; i<dataList.length; ++i) {
						...
						sb.Append("	<td>"+((dataInfo.learnApp.cardinal && dataInfo.learnApp.cardinal.name) ? dataInfo.learnApp.cardinal.name : "")+"</td>");
						...
						sb.Append(" <td>"+(dataInfo.bookType == "1" ? "구매" : "반송")+"</td>");
						sb.Append("	<td>"+dataInfo.bank+"</td>");
						sb.Append("	<td>"+dataInfo.account+"</td>");
						sb.Append("	<td>"+dataInfo.owner+"</td>");
						sb.Append("	<td>"+dataInfo.memo+"</td>");
						sb.Append(" <td>"+(dataInfo.state == "1" ? "취소신청" : (dataInfo.state == "2" ? "취소완료" : "취소불가"))+"</td>");
						sb.Append("	<td>");
						if(dataInfo.state == "1") {
							sb.Append("		<input type='hidden' name='cancelId' value='"+dataInfo.id+"' />");
							sb.Append("		<input type='hidden' name='cancelLearnAppId' value='"+dataInfo.learnApp.id+"' />");
							sb.Append("		<input type='hidden' name='cancelMemo' value='"+dataInfo.memo+"' />");
							sb.Append("		<input type='hidden' name='cancelDate' value='"+regDate+"' />");
							sb.Append("		<a class='btn cancel_btn' href='javascript:void(0);'>취소처리</a>");
						}
						sb.Append("	</td>");
					...
					sb.Append("<tr>");
					sb.Append("	<td colspan='15'>조회된 결과가 없습니다.</td>");
					sb.Append("</tr>");
				}
	...
	// 선택 취소처리 클릭 시
	$("#dataListBody").on("click", ".cancel_btn", function() {
		var idx = $(".cancel_btn").index($(this));
		var data = new Object();

		// 취소신청데이터
		data.id = $(":hidden[name='cancelId']").eq(idx).val();
		data["learnApp.id"] = $(":hidden[name='cancelLearnAppId']").eq(idx).val();
		data["learnApp.reqType"] = "4";
		data["learnApp.reqMemo"] = $(":hidden[name='cancelMemo']").eq(idx).val();
		data["learnApp.reqDate"] = $(":hidden[name='cancelDate']").eq(idx).val();
       		...
       		<div>
	        	<table class="searchTable">
	        		...
	        		<tr>
	        			...
	        			<td>
		        			<!-- 처리상태 -->
							<select class="selector" name='state'>
								<option value="">처리상태</option>
								<option value="1" <c:if test="${search.state eq '1'}">selected</c:if>>취소신청</option>
								<option value="2" <c:if test="${search.state eq '2'}">selected</c:if>>취소완료</option>
								<option value="3" <c:if test="${search.state eq '3'}">selected</c:if>>취소불가</option>
							</select>
	        			</td>
	        		</tr>
	        		<tr>
