				...
				if(result.list && result.list.length > 0) {
					...
					for(var i=0; i<dataList.length; ++i) {
						...
						*/
						...
						sb.Append("	<td>"+dataInfo.appStartDate+" ~ "+dataInfo.appEndDate+"</td>");
		...
		<!-- searchForm start -->
       	<form name="searchForm" method="post" onsubmit="return false;">	
			...
			<select name="">
				...
				<!--  
				<c:forEach items="${subCourseList}" var="subCourse" varStatus="status">
					<option value='<c:out value="${subCourse.code}" />' <c:if test="${subCourse.code eq search.courseCode.code}">selected</c:if>><c:out value="${subCourse.name}" /></option>
				</c:forEach>
				-->
			</select>
			...
			<select name="courseCode.code">
				<option value="">학습영역</option>
				<!--  
				<c:forEach items="${courseList}" var="course" varStatus="status">
					<option value='<c:out value="${course.code}" />' <c:if test="${course.code eq search.courseCode.code}">selected</c:if>><c:out value="${course.name}" /></option>
				</c:forEach>
				-->
			</select>
