...
var listUrl = "<c:url value='/admin/common/smsList'/>";
var detailUrl = "<c:url value='/admin/common/smsHistoryDetail'/>";
...
$(document).ready(function () {
	...
	$('#dataListBody').on('click', ".title", function(){
		var sms_seq = $(this).closest("tr").find("td:eq(0)").find("input.sms_seq").val();
		var mid = $(this).closest("tr").find("td:eq(0)").find("input.sms_mid").val();
		...
		var tag = '<input type="hidden" value="'+ sms_seq +'" name="sms_seq">';
		tag += '<input type="hidden" value="'+ mid +'" name="sms_mid">';
		...
		$('#smsDetailForm').html(tag);
		contentLoad("상세정보조회", detailUrl, $('#smsDetailForm').serialize());
	});
	setContentList(1);
})
//함수영역
		...
		success	: function(result){
			...
			if(result.list && result.list.length > 0) {
				...
				for(var i=0; i<dataList.length; ++i) {
					...
					var sms_seq = dataInfo.SMS_SEQ;
					sb.Append('<tr>');
					sb.Append('<td>' + (i+1));
					sb.Append('<input type="hidden" class="sms_seq" value="'+ dataInfo.SMS_SEQ +'"/>');
					sb.Append('<input type="hidden" class="sms_mid" value="'+ dataInfo.SMS_MID +'"/>');
					sb.Append('</td>');
					sb.Append('<td class="title" style="cursor:pointer; color: #0000ff">'+ dataInfo.SEND_TITLE +'</td>');
					sb.Append('<td>'+ dataInfo.SEND_NAME +'</td>');
					sb.Append('<td>'+ dataInfo.SEND_NUM +'</td>');
					if(dataInfo.RDATE == null || dataInfo.RDATE=='') {
						sb.Append('<td>즉시</td>');
					} else {
						sb.Append('<td>예약</td>');
					}
					sb.Append('<td>'+ dataInfo.SMS_INS_DATE +'</td>');
					sb.Append('<td>'+ dataInfo.SEND_CNT +'</td>');
					sb.Append('<td>');
	...
	$('input[name="detailBtn"]').on('click', function() {
		...
		$.ajax({
			...
			success	: function(result) {
				...
				var data = new Object();
				data.list = JSON.stringify(jsonObj.list);
				openLayerPopup('SMS 상세내역 조회','/admin/common/popup/smsDetail', data);
			},
...
<div class="content_wraper" id="modalsContentss">
	...
	<div class="tab_body">
	        		...
	        		<tr>
	        			...
	        			<td>
	        				<select class="searchConditionBox" name="searchCondition">
								...
								<option value='2' <c:if test="${search.searchCondition eq '2'}">selected</c:if>>발송번호</option>
								<option value='3' <c:if test="${search.searchCondition eq '3'}">selected</c:if>>제목</option>
								<option value='4' <c:if test="${search.searchCondition eq '4'}">selected</c:if>>내용</option>
							</select>
		...
		<form action="/admin/common/smsDetailForm" method="post" name="smsDetailForm" id="smsDetailForm"></form>
	</div>
</div>
