...
$(document).ready(function () {
	/** 변수 영역 **/
	// 여기에서 URL 변경 시켜서 사용
	var listUrl	= "<c:url value='/data/stats/surveyStatsList' />";
	var detailUrl = "<c:url value='/admin/stats/surveyStatsDetail' />";
	...
	$("#dataListBody").on("click", ".content_edit", function() {
		...
		$(":hidden[name='survey.id']").val($(":hidden[name='checkId']").eq(idx).val());
		
		// ajax로 load
		contentLoad("연수설문통계 상세", detailUrl, $("form[name='searchForm']").serialize());
	});
	...
	<div class="tab_body">
		...
		<!-- searchForm start -->
       	...
       	<form name="searchForm" method="post" onsubmit="return false;">
        	...
        	<input type="hidden" name="survey.id" value="" />
        	<!-- pageNo -->
        	<input type="hidden" name="pageNo" value='<c:out value="${search.pageNo}" default="1" />' />
		
			<input type="hidden" name="surveyCode.code" value="${search.survey.surveyCode.code}" />
			<input type="hidden" name="survey.surveyCode.code" value="${search.survey.surveyCode.code}" />
		</form>
		<!-- //searchForm end -->
