...
$(document).ready(function () {
	...
	var replyRegUrl	= "<c:url value='/data/board/boardReplyReg' />";
	var replyUpdUrl	= "<c:url value='/data/board/freeBoardReplyUpdate' />";
	var replyDelUrl	= "<c:url value='/data/board/boardReplyDel' />";
	...
	//댓글영역 숨김
	var boardIdVal = $(":hidden[name=id]").val();
	if(boardIdVal ==0){		
		 $("#cmt_list_area_reply").css("display","none");			
		 $("#cmt_insert_area_reply").css("display","none");			
	}
	...
	// 답글 삭제 
	$("#replyDelBtn").unbind("click").bind("click", function() {
		if(confirm("삭제하시겠습니까?")) {
			$.ajax({
				type	: "post",
				url		: replyDelUrl,
...
<div class="content_wraper">
	...
	<div class="tab_body">
		...
		<form name="replyForm" method="post">
			...
			<input type='hidden' name='boardId' value='<c:out value='${boardReply.boardId}' default="0" />' />
			...
			<dl>
				...
				<dd class="half">
					<div class='attach_file_area' style="clear: both;">
						...
						<input type='hidden' class='attach_file_id'	name='file1.fileId' value='<c:out value="${boardReply.file1.fileId}" default="" />' />
						...
						<c:if test="${boardReply.file1 ne null and boardReply.file1.detailList ne null and fn:length(boardReply.file1.detailList) > 0}">
							<c:forEach items="${boardReply.file1.detailList}" var="file" varStatus="status">
				...
				<dd class="half">
					<div class='attach_file_area' style="clear: both;">
						<!-- fileId 세팅(저장 시 가지고 가야 할 ID) -->
						<input type='hidden' class='attach_file_id'	name='file2.fileId' value='<c:out value="${boardReply.file2.fileId}" default="" />' />
						<input type='hidden' class='upload_dir'	value='/upload/board/files' />
						...
						<c:if test="${boardReply.file2 ne null and boardReply.file2.detailList ne null and fn:length(boardReply.file2.detailList) > 0}">
							<c:forEach items="${boardReply.file2.detailList}" var="file" varStatus="status">
							<div class='file_info_area' style='float: left;'>
		...
		<!-- 댓글입력 -->
		<div id="cmt_insert_area_reply">
		<form name="cmtReplyInsertForm" method="post">
		    <dl>				 
				<dt>댓글</dt>
				<dd> 		
				    <input type="hidden" name="category" value="${category }">	    		    
				    <input type="hidden" name="boardId"  value="${boardReply.id}">
		...
		<!-- 등록된 댓글리스트 -->
		   <div id="cmt_list_area_reply">	 		 	
				<input type="hidden" name="boardId"   value="${boardReply.id}">
					<dl>	
						<c:forEach items="${boardReplyCmtList}" var="boardReplyCmt">
							<dt>${boardReplyCmt.userId}</dt>
							<dd>  							   									 
								<input type="hidden" name="id" class="cmtListId" value="${boardReplyCmt.id}">
								<input type="text" name="comment" class="cmtListCmt form-control w80" value="<c:out value='${boardReplyCmt.comment}'/>" />  
			...
			<a  class=" listBtn btn align_right" href="javascript:void();">리스트</a>
				<c:if test="${boardReply.id !=0}">
					<a id="replyDelBtn" class="btn align_right danger" href="javascript:void();">삭제</a>
					<a id="replyUpdBtn" class="btn align_right primary" href="javascript:void();">수정</a>	
				</c:if>
				<c:if test="${boardReply.id==0}">
				<a id="replyRegBtn" class="btn align_right primary" href="javascript:void();">저장</a>			    
				</c:if>
			</div>
			<!-- view end -->
