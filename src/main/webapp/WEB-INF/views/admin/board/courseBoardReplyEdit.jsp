...
$(document).ready(function () {
		...
		if(false) {
			...
			$.ajax({
				...
				success	: function(result) {
					if(result.id) {
						alert("저장되었습니다.");
						contentLoad("과정별 질의응답관리", listUrl, $("form[name='searchForm']").serialize());
					} else {
						alert("저장실패했습니다.");
					}
				},
	...
	// 컨텐츠 타이틀
	$('.content_wraper').children('h3').eq(0).html("과정별 질의응답관리");
});
...
<div class="content_wraper">
	...
	<div class="tab_body">
		...
		<form name="actionForm" method="post">
			<!-- hidden 데이터 -->
			<input type='hidden' name='id' value='<c:out value='${board.boardReply.id}' default="0" />' />
			...
			<dl>
				<dt>답글제목</dt>
				<dd>
					<input type="text" name="title" class="form-control" value="<c:out value='${board.boardReply.title}'/>" />
				</dd>
				<dt>답글내용</dt>
				<dd>
					<textarea class="editor" id="comment_editor" name="comment" rows="10" cols="100" style="width:100%; height:200px;"><c:out value="${board.boardReply.comment}" /></textarea>
				</dd>
			</dl>
			<!-- view end -->
		</form>
		...
		<div>
			...
			<c:if test="${not empty board.boardReply.id}">
