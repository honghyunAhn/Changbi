    ...
    function html5Upload() {
    	...
    	//sUploadURL = '/upload/course/mainImg';
    	//sUploadURL= 'file_uploader_html5.php'; 	//upload URL
    	sUploadURL = '/forFaith/file/naverSmartEditor';
    	//sUploadURL = '/common/editor/photo_uploader/file_uploader_html5.jsp';
    	...
    	for(var j=0, k=0; j < nImageInfoCnt; j++) {
    		...
    		try{
	    		if(!!tempFile){
	    		    ...
	    		    editorUpload(tempFile, sUploadURL);
	    			k += 1;
	    		}
	    	}catch(e){}
    		tempFile = null;
	...
	function onAjaxError (){
     ...
     function uploadImage (e){
    	 if(!bSupportDragAndDropAPI){
    		 //generalUpload();
    	     var file_data  = getElement("uploadInputBox");
    	     var fileData   = file_data.files[0];
    	     var sUploadURL = '/forFaith/file/naverSmartEditor';     //upload URL

    	     editorUpload(fileData, sUploadURL);
    	 }else{
    		 html5Upload();
    	 }
     }
...
// 	        sCallback : location.href.replace(/\/[^\/]*$/, '') + '/callback.html',	//업로드 이후에 iframe이 redirect될 콜백페이지의 주소
 			...
 			sCallback : '/common/editor/photo_uploader/callback.html',	//업로드 이후에 iframe이 redirect될 콜백페이지의 주소
